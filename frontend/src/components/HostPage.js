...
export default function HostPage() {
  ...
  if (!state.pin) {
    ...
    screen = (
      <div className="h-screen flex flex-col items-center px-4 pt-14 pb-4 overflow-hidden">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-xl mx-auto flex flex-col min-h-0 flex-1"
        >
          {/* Header compacto */}
          <div className="flex items-center justify-between mb-3 flex-shrink-0">
            <h1 className="text-2xl font-black">
              ...
              <span className="text-slate-500 text-sm font-medium ml-2">Professor</span>
            </h1>
            <button
              onClick={() => navigate('/')}
          ...
          {/* Tabs */}
          <div className="flex rounded-xl bg-slate-800/60 border border-slate-700/50 p-1 mb-4 flex-shrink-0">
            <button
          ...
          {/* Error */}
          {uploadError && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-rose-400 text-sm text-center mb-3 flex-shrink-0">
              {uploadError}
            </motion.p>
          )}

          {/* Conteúdo das abas — flex-1 para preencher espaço restante */}
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'import' && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-5"
                >
                  <label className="block mb-4">
                    ...
                    <div className="relative">
                      ...
                      <motion.div
                        ...
                        className="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center hover:border-violet-500 transition-colors cursor-pointer"
                      >
                        {uploading ? (
                          <div className="flex items-center justify-center gap-3">
                            ...
                            <span className="text-slate-300 text-sm">Processando...</span>
                          </div>
                        ) : (
                          <div className="flex items-center justify-center gap-4">
                            <svg className="w-8 h-8 text-slate-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m6.75 12l-3-3m0 0l-3 3m3-3v6m-1.5-15H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                            </svg>
                            <div className="text-left">
                              <p className="text-slate-300 font-medium text-sm">Clique ou arraste o arquivo</p>
                              <p className="text-slate-500 text-xs">Apenas .docx — máx. 5MB</p>
                            </div>
                          </div>
                        )}
                      </motion.div>
...
c) Brasília (correta)  d) Salvador`}
              ...
              {activeTab === 'create' && (
                ...
                >
                  {/* Formulário — compacto */}
                  <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 flex-shrink-0">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs text-slate-500 uppercase tracking-wider">
                        {editingIndex !== null ? `Editando questão ${editingIndex + 1}` : 'Nova questão'}
                      </p>
                      {editingIndex !== null && (
                        <button onClick={resetEditingQ} className="text-xs text-slate-500 hover:text-slate-300 transition-colors">
                          Cancelar
                        </button>
                      )}
                    </div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-800/60 border border-slate-700/50 rounded-lg px-3 py-2 text-white placeholder-slate-600 text-sm focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30 transition-all mb-3"
                    />

                    {/* Options — layout compacto em grid 2 colunas */}
                      ...
                      {editingQ.options.map((opt, i) => (
                        <div key={i} className="flex items-center gap-1.5">
                          <button
                            ...
                            className={`flex-shrink-0 w-7 h-7 rounded-md border-2 flex items-center justify-center transition-all text-xs ${
                          ...
                          <input
                            ...
                            className="flex-1 min-w-0 bg-slate-800/60 border border-slate-700/50 rounded-lg px-2.5 py-1.5 text-white placeholder-slate-600 text-sm focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30 transition-all"
                          />
                          {editingQ.options.length > 2 && (
                            <button onClick={() => removeOption(i)} className="flex-shrink-0 text-slate-600 hover:text-rose-400 transition-all">
                    ...
                    <div className="flex items-center gap-2">
                      {editingQ.options.length < 5 && (
                        <button onClick={addOption} className="text-xs text-slate-500 hover:text-violet-400 transition-colors flex items-center gap-1">
                      ...
                      <motion.button
                        ...
                        disabled={!canSaveQ}
                        whileTap={{ scale: 0.97 }}
                        className="py-2 px-5 rounded-lg font-medium text-sm bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center gap-1.5"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {/* Lista de questões — área scrollável */}
                  {manualQuestions.length > 0 && (
                    <div className="mt-3 bg-slate-900/60 border border-slate-700/50 rounded-2xl p-3 flex-1 min-h-0 flex flex-col">
                      <p className="text-xs text-slate-500 mb-2 flex items-center gap-1.5 flex-shrink-0">
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <div className="space-y-1.5 overflow-y-auto flex-1 min-h-0">
                        {manualQuestions.map((q, i) => (
                          ...
                          >
                            <span className="w-5 h-5 rounded bg-slate-700/60 flex items-center justify-center text-[10px] font-bold text-slate-400 flex-shrink-0">
                              {i + 1}
                            </span>
                            <span className="flex-1 text-slate-300 truncate text-xs">{q.text}</span>
                            <span className="text-[10px] text-emerald-500 flex-shrink-0 font-medium">{LETTERS[q.correctIndex]})</span>
                            <button onClick={() => editQuestion(i)} className="flex-shrink-0 text-slate-500 hover:text-violet-400 transition-all" title="Editar">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                              </svg>
                            </button>
                            <button onClick={() => removeQuestion(i)} className="flex-shrink-0 text-slate-500 hover:text-rose-400 transition-all" title="Remover">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className="w-full py-3 mt-3 rounded-xl font-bold text-sm bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 flex-shrink-0"
                  >
                    {uploading ? (
                      <>
  ...
  // ── Lobby ──
    ...
    screen = (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 pt-16 pb-6">
        <motion.div
